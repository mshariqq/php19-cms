    ...
    <div class="row">
        ...
        <div class="col-md-10 bg-dark-light text-white pt-4 pb-5">
            ...
            <nav class="breadcrumb text-light text-white">
                ...
                <a class="breadcrumb-item text-grey text-warning" href="{{ route('admin.tags') }}">Tags</a>
            </nav>
            ...
            <hr>     
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($cats as $item)
                                <tr>
                                    <td>
                                        <a href="{{ route('tags.posts' , $item->slug) }}">{{$item->tag}}</a>
                                    </td>
                                    <td>{{$item->slug}}</td>
                                    <td>
                                        {{-- <a href="{{route('admin.pages.edit', $item->id)}}" class="btn btn-sm btn-primary">
                                        <i class="fa fa-pencil" aria-hidden="true"></i></a> --}}
                                        <a href="{{route('admin.tags.delete', $item->id)}}" class="btn btn-sm btn-danger">
                ...
                @else 
                    <p class="text-warning">No tags found</p>
                @endif

                <hr>
                <form action="{{route('admin.tags.store')}}" method="post" class="col-md-4">
                    @csrf
